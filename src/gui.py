...
def launch_gui() -> None:
    ...
    def run() -> None:
        ...
        needs_ref = get_definition(workflow_id).requires_reference
        if not state["raw"]:
    ...
    ttk.Label(root, textvariable=out_var, width=45, anchor="w").grid(
        row=4, column=1, columnspan=2, **pad
    )

    ttk.Label(root, text="SEGMEN filter (optional):").grid(row=5, column=0, **pad)
    ...
    ttk.Button(root, text="Run Pipeline", command=run, width=22).grid(
        row=6, column=0, columnspan=3, **pad
    )

    def toggle_reference_row(*_args: object) -> None:
        """Show the reference-file row only for workflows that require it."""
        if get_definition(selected_workflow_id()).requires_reference:
            ref_button.grid()
            ref_label.grid()
        else:
    ...
    workflow_combo.bind("<<ComboboxSelected>>", toggle_reference_row)
    toggle_reference_row()  # apply initial visibility for the default workflow
